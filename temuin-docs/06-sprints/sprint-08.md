...
| DO-8.3 | Final deployment ke Tencent VPS dengan tag v1.0.0 + cleanup environment | High | 2h | DO-8.2, BE-8.6 | done | tag v1.0.0 | Deploy via CD auto-deploy (merge #122) + verified: 6 container healthy non-root, `/api/status` 3 up, `/` 200. Disk cleanup 70%->55% (prune dangling+build cache). Tag v1.0.0 pushed + GitHub Release dibuat |
...
| DO-8.5 | Git tag v1.0.0 + buat `docs/release-notes.md` (highlights, migration notes, known limitations) | Medium | 1h | DO-8.3 | done | tag v1.0.0 | `docs/release-notes.md` merged (PR #122). Tag v1.0.0 (annotated) pushed ke remote + GitHub Release published |
